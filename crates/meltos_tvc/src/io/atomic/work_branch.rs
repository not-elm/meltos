    ...
    Fs: FileSystem,
{
    ...
    pub async fn write(&self, branch_name: &BranchName) -> error::Result<()> {
        self.0
            .write_file(".meltos_core/WORKING", &serde_json::to_vec(branch_name)?)
            .await?;
        Ok(())
    }
    ...
    pub async fn try_read(&self) -> error::Result<BranchName> {
        let buf = self.0.try_read_file(".meltos_core/WORKING").await?;
        Ok(serde_json::from_slice(&buf)?)
    }

    #[inline]
    pub async fn read(&self) -> error::Result<Option<BranchName>> {
        let Some(buf) = self.0.read_file(".meltos_core/WORKING").await? else {
            return Ok(None);
        };
